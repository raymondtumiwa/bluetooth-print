...
class HomeCompletedTask extends StatefulWidget {
  HomeCompletedTask({Key key, this.title}) : super(key: key);

  final String title;

  @override
  _HomeCompletedTaskState createState() => _HomeCompletedTaskState();
}

class _HomeCompletedTaskState extends State<HomeCompletedTask> {
  Stream _future;
  var orderan = [];
  int tukangEs = 10;
  String username;
  ...
  setUpTimedFetch(username, employeeRole, employeeArea, tokens) {
    ...
    Timer.periodic(Duration(milliseconds: 1000), (timer) {
      if (mounted) {
        ...
        OnRest.instance
            .findCompletedTask(username, tokens)
            .then((value) => setState(() {
                  //print("disini?");
                  //print(value);
                  if (value.isEmpty) {
